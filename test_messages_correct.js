...
async function testSendMessage() {
  ...
  if (!tablesExist[2]) {
    ...
    console.log('📋 确保使用正确的 Supabase 项目: https://zbhlrnecjmdpuaxvhneu.supabase.co');
    return;
  }
  ...
  try {
    ...
    console.log('\n🎉 消息功能测试通过！');
    console.log('💡 现在可以在小程序中正常使用联系卖家功能了');
    
  } catch (error) {
    console.error('❌ 测试失败:', error.message);
    console.error('🔍 请检查:');
    console.error('   1. 是否已执行 complete_message_fix.sql');
    console.error('   2. 数据库中是否有足够的测试数据');
